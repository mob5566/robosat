...
class BaseHandler:
    def __init__(self, ksize_denoise, ksize_grow, sim_threshold, zoom_level):
        self.features = []
        self.kernel_size_denoise = ksize_denoise
        self.kernel_size_grow = ksize_grow
        self.simplify_threshold = sim_threshold
        self.zoom_level = zoom_level

    def apply(self, tile, mask):
        if tile.z != self.zoom_level:
            raise NotImplementedError("Post-processing thresholds are tuned for z{self.zoom_level}")
